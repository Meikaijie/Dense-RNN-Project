...
for current_input in inputs_series:
    ...
    to_concat.append(tf.squeeze(tf.slice(current_state, [skip_connections[num_skip_connections-1] - 1, 0, 0], [1, batch_size, state_size]), axis=[0]))
    input_and_state_concatenated = tf.concat(to_concat, 1)  # Increasing number of columns

    next_state = tf.tanh(tf.matmul(input_and_state_concatenated, W) + b)  # Broadcasted addition
    toDivide = 1
    for i in range(1, num_skip_connections):
        toDivide = toDivide + 1
        next_state = next_state + tf.squeeze(tf.slice(current_state, [skip_connections[num_skip_connections-1] - skip_connections[i], 0, 0], [1, batch_size, state_size]), axis=[0])
    next_state = next_state / toDivide
