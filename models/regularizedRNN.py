...
def drnn_classification(x,
    ...
    if dilations[0] == 1:
        ...
        weights = tf.Variable(tf.random_normal(shape=[hidden_structs[-1] * (len(regularizers) + 1),
                                                      n_classes]))
        bias = tf.Variable(tf.random_normal(shape=[n_classes]))

        preds = []
        for i in range(len(layer_outputs)):
            to_concat = [layer_outputs[i]] 
...
logits_series = drnn_classification(x_input, [state_size], [1], regularizers, truncated_backprop_length, num_classes, 1)
