...
batchX_placeholder = tf.placeholder(tf.float32, [None, truncated_backprop_length,123])
...
logits_series = drnn_classification(x_input, [state_size] * layers, dilations[0:layers], truncated_backprop_length, num_classes, 123)
        ...
        for batch_idx in range(num_batches):
            ...
            for f in os.listdir('feature_files/TRAIN'):
                if file_pos == counter:
                    ...
                    label = np.load(os.path.join('feature_labels/TRAIN',f[:-7]+'npy'))
                    file_pos += 1
                    break
                counter += 1
            ...
            for batch_pos in range(stop_index):
                ...
                # print(len(example))
                # print(len(batchX))
                # print(batch_pos//truncated_backprop_length)
